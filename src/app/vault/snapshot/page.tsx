    ...
    return (
      <main className="min-h-screen bg-black text-white px-6 py-16">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="bg-gradient-to-br from-pink-500/10 to-purple-500/10 rounded-xl p-8 border border-white/10 text-center">
            <div className="text-6xl mb-4">📸</div>
            <h2 className="text-2xl font-semibold mb-4">Your Snapshot is Empty</h2>
            <p className="text-white/60 mb-6">
              Start building your health identity by uploading documents, chatting with Beau,
              or adding your first provider.
            </p>
    ...
    <main className="min-h-screen bg-black text-white px-6 py-16">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <h1 className="text-4xl font-bold mb-2">{snapshot?.vaultName}</h1>
        ...
        {snapshot && (
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-8">
            <div className="bg-gradient-to-br from-blue-500/10 to-cyan-500/10 rounded-lg p-4 border border-white/10">
              <div className="text-2xl font-bold">{snapshot.stats.documents}</div>
            ...
            <div className="bg-gradient-to-br from-purple-500/10 to-pink-500/10 rounded-lg p-4 border border-white/10">
              <div className="text-2xl font-bold">{snapshot.stats.chats}</div>
            ...
            <div className="bg-gradient-to-br from-green-500/10 to-emerald-500/10 rounded-lg p-4 border border-white/10">
              <div className="text-2xl font-bold">{snapshot.stats.appointments}</div>
            ...
            <div className="bg-gradient-to-br from-pink-500/10 to-red-500/10 rounded-lg p-4 border border-white/10">
              <div className="text-2xl font-bold">{snapshot.stats.providers}</div>
            ...
            <div className="bg-gradient-to-br from-yellow-500/10 to-orange-500/10 rounded-lg p-4 border border-white/10">
              <div className="text-2xl font-bold">{snapshot.stats.decoded}</div>
        ...
        {snapshot?.upcomingAppointments && snapshot.upcomingAppointments.length > 0 && (
          <div className="mb-8">
            <h2 className="text-2xl font-semibold mb-4 flex items-center gap-2">
              <span>📅</span> Upcoming Appointments
            </h2>
            <div className="space-y-3">
              {snapshot.upcomingAppointments.map((apt) => (
                <div
                  key={apt.id}
                  className="bg-gradient-to-br from-green-500/10 to-blue-500/10 rounded-lg p-4 border border-white/10 hover:border-green-400/30 transition"
                >
                  <div className="flex items-start justify-between">
                    <div>
                      <h3 className="font-semibold">{apt.providerName}</h3>
                      {apt.specialty && (
                        <p className="text-sm text-white/60">{apt.specialty}</p>
                      )}
                    </div>
                    <div className="text-right">
                      <div className="font-medium">{apt.date}</div>
                      <div className="text-sm text-white/60">{apt.time}</div>
                    </div>
                  </div>
                  <div className="text-xs bg-white/5 rounded px-2 py-1 inline-block mt-2">
                    {apt.type}
                  </div>
                </div>
              ))}
        ...
        {snapshot?.providers && snapshot.providers.length > 0 && (
          <div className="mb-8">
            <h2 className="text-2xl font-semibold mb-4 flex items-center gap-2">
              <span>👩‍⚕️</span> Your Care Team
            </h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {snapshot.providers.map((provider) => (
                <div
                  key={provider.id}
                  className="bg-gradient-to-br from-purple-500/10 to-pink-500/10 rounded-lg p-4 border border-white/10"
                >
        ...
        {/* Goals Section */}
        {snapshot?.goals && Array.isArray(snapshot.goals) && snapshot.goals.length > 0 && (
