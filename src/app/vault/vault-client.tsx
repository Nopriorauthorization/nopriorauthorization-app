...
export default function VaultClient({ initialData }: VaultClientProps) {
  const [data, setData] = useState<VaultData | null>(initialData);
  const [showOnboarding, setShowOnboarding] = useState(false);
  const [filter, setFilter] = useState<"all" | "instant" | "capture" | "power">("all");

  useEffect(() => {
    setData(initialData);
    setShowOnboarding(Boolean(initialData && !initialData.vaultName));
  }, [initialData]);
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="rounded-2xl border border-pink-400/30 bg-pink-400/5 p-8 mb-8">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <Link
                href="/vault/decoder"
                className="p-4 rounded-xl border border-white/10 bg-white/5 hover:border-pink-400/30 hover:bg-white/10 transition"
              >
                ...
                <p className="text-sm text-gray-400">
                  Scan a prescription, lab result, or bill to get started
                </p>
              </Link>
              <Link
                href="/vault/snapshot"
          ...
          <div className="flex flex-wrap gap-3 mb-8 border-b border-white/10 pb-4">
            <button
              ...
              className={`px-4 py-2 rounded-full text-sm font-semibold transition ${
                filter === "all" ? "bg-pink-400 text-black" : "bg-white/5 text-white/70 hover:bg-white/10"
              }`}
            >
              All Features
            </button>
            <button
              ...
              className={`px-4 py-2 rounded-full text-sm font-semibold transition ${
                filter === "instant" ? "bg-pink-400 text-black" : "bg-white/5 text-white/70 hover:bg-white/10"
              }`}
            >
              ⚡ Instant Access
            </button>
            <button
              ...
              className={`px-4 py-2 rounded-full text-sm font-semibold transition ${
                filter === "capture" ? "bg-pink-400 text-black" : "bg-white/5 text-white/70 hover:bg-white/10"
              }`}
            >
              📸 Smart Capture
            </button>
            <button
              ...
              className={`px-4 py-2 rounded-full text-sm font-semibold transition ${
                filter === "power" ? "bg-pink-400 text-black" : "bg-white/5 text-white/70 hover:bg-white/10"
              }`}
            >
              🎯 My Resources
            </button>
          ...
          <div className="mt-12 rounded-xl border border-white/10 bg-white/5 p-6">
            <p className="text-sm text-gray-400">
              <strong className="text-white">Your data. Your control.</strong> Everything in your
              Sacred Vault is encrypted, private, and accessible only to you. Share access with
              providers or family on your terms — with granular control over what they see.
            </p>
          </div>
        </div>
      </main>
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          <div className="rounded-xl border border-white/10 bg-white/5 p-4">
            <div className="text-3xl font-bold text-pink-400">{data.stats.documents}</div>
            <div className="text-sm text-gray-400 mt-1">Documents</div>
          </div>
          <div className="rounded-xl border border-white/10 bg-white/5 p-4">
            <div className="text-3xl font-bold text-pink-400">{data.stats.appointments}</div>
            <div className="text-sm text-gray-400 mt-1">Appointments</div>
          </div>
          <div className="rounded-xl border border-white/10 bg-white/5 p-4">
            <div className="text-3xl font-bold text-pink-400">{data.stats.chats}</div>
            <div className="text-sm text-gray-400 mt-1">Conversations</div>
          </div>
          <div className="rounded-xl border border-white/10 bg-white/5 p-4">
            <div className="text-3xl font-bold text-pink-400">{data.stats.decoded}</div>
            <div className="text-sm text-gray-400 mt-1">Decoded</div>
          </div>
        </div>
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {filteredFeatures.map((feature) => {
            ...
            return isActive ? (
              ...
              >
                {feature.hasData && (
                  <div className="absolute top-4 right-4 w-2 h-2 rounded-full bg-green-400"></div>
                )}
                <div className="flex items-start gap-3 mb-3">
                  <span className="text-3xl">{feature.icon}</span>
                  <div className="flex-1">
