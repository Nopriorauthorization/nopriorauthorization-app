...
export interface MascotDomainShellProps {
  ...
  videoSrc?: string;
  videoPoster?: string;
  showVideo?: boolean; // default: false - controls video visibility

  // Features (max 4 displayed)
...
export default function MascotDomainShell({
  ...
  // Get theme from mascotId - no manual color props needed
  ...
  const onPlay = () => {
    ...
    mediaController.play(v, mascotId);
  ...
  // Standardized chat navigation: /chat?mascot=X&source=Y
  const onAsk = () => {
    const chatSource = source || mascotId;
    router.push(`/chat?mascot=${encodeURIComponent(mascotId)}&source=${encodeURIComponent(chatSource)}`);
  };
